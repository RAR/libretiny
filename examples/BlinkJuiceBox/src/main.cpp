 ...
 *
 * Walks the three bench-confirmed RGB channels — LED_R=PB3, LED_G=PB5,
...
void setup() {
	Serial.begin(115200);
	Serial.println("JuiceBox 40 WGM160P — RGB cycle (R=PB3 G=PB5 B=PD8)");
	for (int i = 0; i < 3; i++)
		pinMode(ch[i], OUTPUT);
}

void loop() {
	const char *name[3] = {"red (PB3)", "green (PB5)", "blue (PD8)"};
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++)
